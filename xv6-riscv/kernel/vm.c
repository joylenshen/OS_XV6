...
// Lab cow: 使用引用计数，只有引用计数降到0时才真正释放物理页
void
uvmunmap(pagetable_t pagetable, uint64 va, uint64 npages, int do_free)
{
  ...
  for (a = va; a < va + npages * PGSIZE; a += PGSIZE) {
    ...
    if (do_free) {
      ...
      // Lab cow: 使用引用计数释放，只有引用计数降到0时才释放
      kfree((void *)pa);
    }
    *pte = 0;
  }
...
// Lab cow: 改为写时复制（Copy-on-Write），不复制物理页，父子共享只读页
int
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for (i = 0; i < sz; i += PGSIZE) {
    ...
    // Lab cow: 清除写权限(PTE_W)，只保留读权限(PTE_R)
    // 父子进程共享同一物理页，都是只读
