...
// returns 0 if out of disk space.
// Lab fs: 支持双重间接块（扩展最大文件大小）
static uint
bmap(struct inode *ip, uint bn)
{
  uint addr, *a, *indirect2;
  struct buf *bp, *bp2;
  ...
  if (bn < NDOUBLE * NINDIRECT) {
    // Lab fs: 双重间接块
    // ip->addrs[NDIRECT+1] 是双间接块的地址
    ...
    if ((addr = indirect2[level2]) == 0) {
      addr = balloc(ip->dev);
      if (addr) {
        indirect2[level2] = addr;
        log_write(bp2);
      }
    }
    brelse(bp2);
