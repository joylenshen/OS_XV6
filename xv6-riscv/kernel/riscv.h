...
// read fp, the frame pointer register (s0)
static inline uint64
r_fp()
{
  uint64 x;
...
// Lab pgtbl: PTE_A = accessed bit (hardware sets on read/write)
#define PTE_A (1L << 6)
// Lab cow: PTE_COW = copy-on-write (custom, use reserved RSW bits)
// PTE_RSW (bits 8-9) reserved for OS use
