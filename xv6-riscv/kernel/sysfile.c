...
sys_open(void)
{
  ...
  // Lab fs: 跟随符号链接（防循环，最多 10 层）
  int depth = 0;
  while (ip->type == T_SYMLINK) {
    // 如果设置了 O_NOFOLLOW，且当前是符号链接，直接返回失败
    if (omode & O_NOFOLLOW) {
      iunlockput(ip);
      end_op();
      return -1;
  ...
  if (!(omode & O_NOFOLLOW)) {
    ...
    while (ip->type == T_SYMLINK) {
      ...
      char target[MAXPATH];
      memset(target, 0, sizeof(target));
      int tn = readi(ip, 0, (uint64)target, 0, MAXPATH - 1);
      iunlockput(ip);
      if (tn < 0) {
        end_op();
        return -1;
...
sys_symlink(void)
{
  ...
  begin_op();
  // 创建新 inode，类型为 T_SYMLINK
  ip = create(path, T_SYMLINK, 0, 0);
  if (ip == 0) {
    end_op();
    return -1;
  }
  // 把 target 写入 inode 数据块
